...
//                new StringReader("{ \"a\" : \"b\", \"c\" : null, \"d\" : [null, \"abc\"] }"));
        new SimpleStringReader("{ \"a\" : \"b\", \"c\" : null, \"d\" : [null, \"abc\"] }"));
        reader.close();
    }

//    public void testReader() {
//        String s = "{ \"a\" : \"b\", \"c\" : null, \"d\" : [null, \"abc\"] }";
////        InputStream is = new ByteArrayInputStream("hi I am test".getBytes(Charset.forName("UTF-8")));
//        InputStream is = new ByteArrayInputStream(s.getBytes());
//
//        JsonParser reader = Json.createParser(is);
//        reader.close();
//    }
...
//    }
    ...
    static void _testWiki(JsonParser parser) {
        Event event = parser.next();
        assertEquals(Event.START_OBJECT, event);

        _testObjectStringValue(parser, "firstName", "John");
        _testObjectStringValue(parser, "lastName", "Smith");
...
//        assertEquals(25, parser.getBigDecimal().intValue());
        ...
        _testObjectStringValue(parser, "streetAddress", "21 2nd Street");
        _testObjectStringValue(parser, "city", "New York");
        _testObjectStringValue(parser, "state", "NY");
        _testObjectStringValue(parser, "postalCode", "10021");
        ...
        event = parser.next();
        assertEquals(Event.START_OBJECT, event);
        _testObjectStringValue(parser, "type", "home");
        _testObjectStringValue(parser, "number", "212 555-1234");
        event = parser.next();
        assertEquals(Event.END_OBJECT, event);

        event = parser.next();
        assertEquals(Event.START_OBJECT, event);
        _testObjectStringValue(parser, "type", "fax");
        _testObjectStringValue(parser, "number", "646 555-4567");
        event = parser.next();
        assertEquals(Event.END_OBJECT, event);
        event = parser.next();
    ...
    static void _testObjectStringValue(JsonParser parser, String name, String value) {
        Event event = parser.next();
        assertEquals(Event.KEY_NAME, event);
